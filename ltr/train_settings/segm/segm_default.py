...
def run(settings):
    ...
    settings.batch_size = 64  # Batch size 缓存大小
    settings.num_workers = 1  # Number of workers for image loading
    settings.normalize_mean = [0.485, 0.456, 0.406]  # Normalize mean (default pytorch ImageNet values) 均值归一化
    settings.normalize_std = [0.229, 0.224, 0.225]  # Normalize std (default pytorch ImageNet values)  STD归一化
    settings.search_area_factor = 4.0  # Image patch size relative to target size 搜索区域相对于目标区域的大小
    settings.feature_sz = 24  # Size of feature map   特征图大小
    settings.output_sz = settings.feature_sz * 16  # Size of input image patches 子图像块
...
#################################3
    ...
    # Learning rate scheduler 学习速率调度器
