...
#####################我写的
def fit_RA_box(mask, rotated=True):
    #对分割后的二值图像寻找轮廓
    if cv2.__version__[-5] == '4':
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    else:
        _, contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    #找出轮廓面积最大的多边形，并使用cv2.fitEllipse进行拟合
    cnt_area = [cv2.contourArea(cnt) for cnt in contours]
...
#            if axes[0] > mask.shape[0] * 0.6 or axes[1] > mask.shape[1] * 0.6:
        ...
        ra_box = fit_RA_box(rotated_mask)

        #aa_poly = np.array(aa_box)
        # print('aa_box is')
        # print(np.array(aa_box))

        # print('ra_box is')
        # print(np.array(ra_box))
