...
######################
    def _crop_back(self, image, bbox, padding=0): #裁切
        # a = (out_sz[0] - 1) / bbox[2]   
        # b = (out_sz[1] - 1) / bbox[3]
        # c = -a * bbox[0]
        # d = -b * bbox[1]
        ...
        # 它是一种二维坐标到二维坐标之间的线性变换，保持二维图形的”平直性”
        # (straightness，即变换后直线还是直线不会打弯，圆弧还是圆弧)和”平行性”(parallelness，保持二维图形间的相对位置关系不变，平行线还是平行线，而直线上点的位置顺序不变，但向量间夹角可能会发生变化)。
        # """ 一个任意的仿射变换都能表示为：乘以一个矩阵(线性变换)接着再加上一个向量(平移)。仿射变换能够表示：旋转(rotation)、平移(translation)、缩放(scale)、翻转(flip)和错切(剪切、shear)操作。
        im_h,im_w = image.shape[:2]

        pts1 = np.float32(list(bbox))
        pts2 = np.float32([[0,0],[im_w,0],[0,im_h],[im_h,im_w]])#变换后矩阵角点位置

        # 生成透视变换矩阵；进行透视变换
        M = cv2.getPerspectiveTransform(pts1, pts2)
        crop = cv2.warpPerspective(image, M, (1920,1080))
    ...
    def track(self, image):
        ...
        if self.segmentation_task or (
            ...
            pred_segm_region,prbox,mask_in_img = self.segment_target(image, new_pos, self.target_sz)
            #pred_segm_region = self.segment_target(image, new_pos, self.target_sz)
            
            
            
            # print('This is pred_segm_region:')
            # print(pred_segm_region)
        ...
        if self.params.use_segmentation:
            if pred_segm_region is not None:
                ...
                return pred_segm_region,prbox,mask_in_img###################
    ...
    def segment_target(self, image, pos, sz):
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:  # 1000:
            ...
            # minAreaRect = cv2.minAreaRect(polygon)
            ...
            #********************代码解释*************************#
            #cv.minarearect面积最小的旋转矩形。返回的是box2d结构，包含[0]最小外接矩形的中心（x，y），[1]矩形宽高和[2]旋转角度
            #cv2.boxpoint获得绘制这个矩形的四个角点

            prbox_init = copy.deepcopy(prbox)

            # print(prbox_init)
            # draw_polygon =cv2.boxPoints(cv2.minAreaRect(polygon))
            # draw_polygon = draw_polygon.flatten().tolist()
            # print(draw_polygon)
            # draw_polygon = np.array(draw_polygon).astype(np.int32)
            # print(draw_polygon.reshape((-1,1,2)))
            # cv2.polylines(image,[draw_polygon.reshape((-1,1,2))],True,(0,255,0),3)
            ...
            if self.params.segm_optimize_polygon:
                if not self.segmentation_task:
                    ...
                    cv2.polylines(image,[contour],True,(0,255,0),3)
            ...
            if not self.params.segm_scale_estimation or pixels_ratio < self.params.segm_pixels_ratio:
                ...
                mask_in_img = self._crop_back(image,prbox)
                return pred_region,prbox,mask_in_img      #####################

        return None

    def poly_to_aabbox(self, x_, y_):
        # keep the center and area of the polygon
        # change aspect ratio of the original bbox 改变原bbox的纵横比
        cx = np.mean(x_)
        cy = np.mean(y_)
        x1 = np.min(x_)
        x2 = np.max(x_)
